...
// - Credentials:
//   - "sewdl-jwt-secret" (Secret text)
//   - "sewdl-ansible-ssh" (SSH private key for app host)
// - Jenkins must have Node.js available for the Build stage (so `npm ci` works).
// - Inventory file should exist at `ansible/inventory/production.ini`.
//
// Note: backend uses `CORS_ORIGIN` env var; set it to your app’s public origin.
...
pipeline {
    ...
    parameters {
        string(
            ...
            defaultValue: 'http://13.201.36.66',
